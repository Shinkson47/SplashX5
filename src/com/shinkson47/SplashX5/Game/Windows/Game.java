...
public class Game {
	...
	public static void Selector(MouseEvent arg0, boolean b) {
		...
		selector = b;
		if (arg0.getButton() == MouseEvent.BUTTON1) { lmb = b; }
		if (arg0.getButton() == MouseEvent.BUTTON3) { rmb = b; }
		
		if (!lmb && !rmb) { selector = false; return; } 
		...
		if (b) {
			...
			try {
			if (CurrentMap.TileSet[selectorx][selectory].ForeTile != null) {selector = false;}
			...
			if (lmb) { Player.Harvest(selectorx, selectory); }
			if (rmb) { Player.Place(selectorx, selectory); }
			
			
			if (!selector) {lmb = false; rmb = false;}
		}	else {
			//release event
		}
		}		
	...
	private static int getSelectorPosY(MouseEvent arg0) {
		...
		if (i > Player.players[Client.PlayerID].Y + selectorRad) { i = Player.players[Client.PlayerID].Y + selectorRad; }
		if (i < Player.players[Client.PlayerID].Y - selectorRad) { i = Player.players[Client.PlayerID].Y - selectorRad; }
		return i;
	}
	...
	private static int getSelectorPosX(MouseEvent arg0) {
		...
		if (i > Player.players[Client.PlayerID].X + selectorRad) { i = Player.players[Client.PlayerID].X + selectorRad; }
		if (i < Player.players[Client.PlayerID].X - selectorRad) { i = Player.players[Client.PlayerID].X - selectorRad; }
		return i;
	}
	...
	public static void RenderFrame() {
		...
		for (int x = 0; x <= (int) (ClientWindow.window.getWidth() / (Game.TileSize)); x++) {
		for (int y = 0; y <= (int) (ClientWindow.window.getHeight() / (Game.TileSize - Game.yoff)) ; y++) {
		...
		try {
			...
			//If not, is the blank tile within the world?
				if (x + Game.DisplayOffsetX > 0 && y + Game.DisplayOffsetY > 0 && x < CurrentMap.WorldBorder && y < CurrentMap.WorldBorder) {
					...
					if (!mape) {
						//if not, create one. The following loops will draw the rest of the chunk this is generated here in this frame, if it is on screen
					...
					}// else ignore it, the player likely removed the tile - that's okay.
				}
			}
			} catch (Exception e) {
