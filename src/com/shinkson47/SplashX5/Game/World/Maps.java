...
public class Maps implements Runnable {
	...
	public static void GenerateNew() {
			...
			Game.CurrentMap.ChunkExists = new boolean[Game.CurrentMap.WorldBorder / Game.ChunkSize][Game.CurrentMap.WorldBorder / Game.ChunkSize];
			...
			for (int width = 0; width < Game.CurrentMap.WorldBorder / Game.ChunkSize; width++) {
				for (int height = 0; height < Game.CurrentMap.WorldBorder / Game.ChunkSize; height++) {
					if (Player.BoundCheck(width * Game.ChunkSize,height * Game.ChunkSize)) { Game.CurrentMap.CharStartX = width; Game.CurrentMap.CharStartY = height; }			
				}	
			}		
			...
			if ((Game.CurrentMap.CharStartX == -1 && Game.CurrentMap.CharStartY == -1)) {
				...
				while(Game.CurrentMap.CharStartX == -1 && Game.CurrentMap.CharStartY == -1) {
					...
					if (Player.BoundCheck(x * Game.ChunkSize,y* Game.ChunkSize)) { Game.CurrentMap.CharStartX = x* Game.ChunkSize; Game.CurrentMap.CharStartY = y* Game.ChunkSize; }
					} catch (Exception e) {
