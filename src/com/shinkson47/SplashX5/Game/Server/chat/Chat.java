...
public class Chat {
	...
	public static String ParseCommand(MessageBase messageBase) {
		...
		case "/field":
			...
			if (varclass == null) { return "Class could not be parsed"; }
			...
			if (field == null) { return "Field could not be parsed"; }
		...
		case "/Load":
			if (args.length < 2) { return "'Load' requires 'name'"; }
			WorldLoader.Load(args[1]);
			return "Parsed load";
		

		case "/Save":
			if (args.length < 2) { return "'Save' requires 'name'"; }
